...
import GoogleImageSearchUtil from '../../utils/googleImageSearchUtil';
import runGeneratorUtil from '../../utils/runGeneratorUtil';
import { cleanString } from '../../utils/stringUtil';
import config from '../../config';
import 'babel-polyfill';

module.exports = class GoogleImageController {

  constructor() {
    this.mongoUtil = new MongoUtil(config.get('db.url'));
    this.googleImageSearch = new GoogleImageSearchUtil();
  ...
  */
  askGoogle(placeStart, placeEnd) {
    (runGeneratorUtil(function *() {
      try {
        console.log('==== init google image search ====');
        ...
        // make a request for each place to google image search
        ...
        // end process
        console.log('==== end google image search ====');
      } catch (exception) {
  ...
  requestPlacesImages(places) {
    ...
    return places.map((place) => new Promise((resolve) => {
      setTimeout(() => {
        ...
        resolve(this.googleImageSearch.search(query, searchOptions));
